...
#SBATCH -J eval_multitask_md

echo "STARTING MULTITASK MULTIDECODER EVAL JOB"
...
eval_log=$LOG_DIR/eval_multitask_md.log
if [ -f $eval_log ]; then
    # Move old log
    mv $eval_log $LOG_DIR/eval_multitask_md-$(date +"%F_%T%z").log
fi

# Decode and evaluate the multitask multidecoder
for source_class in clean dirty; do
    ...
    for subdir in dev test; do
        ...
        dir=$LOG_DIR/multitask_md/$subdir/$source_class
        mkdir -p $dir
        ...
        if [ ! -f $dir/lat.1.gz ]; then
            ...
            $UTILS/run.pl JOB=1:$num_jobs $dir/decode-JOB.log \
                python3 models/scripts/eval_multitask_md.py $feats/$subdir/feats-norm.JOB.scp \
...
echo "DONE MULTITASK MULTIDECODER EVAL JOB"
