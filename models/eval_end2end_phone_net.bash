...
#SBATCH -J eval_end2end_phone_net

echo "STARTING END-TO-END PHONE NET EVAL JOB"
...
eval_log=$LOG_DIR/eval_end2end_phone_net.log
if [ -f $eval_log ]; then
    # Move old log
    mv $eval_log $LOG_DIR/eval_end2end_phone_net-$(date +"%F_%T%z").log
fi

# Decode and evaluate the end2end_phone network
for source_class in clean dirty; do
    ...
    for subdir in dev test; do
        ...
        dir=$LOG_DIR/end2end_phone_net/$subdir/$source_class
        mkdir -p $dir
        ...
        if [ ! -f $dir/lat.1.gz ]; then
            ...
            $UTILS/run.pl JOB=1:$num_jobs $dir/decode-JOB.log \
                python3 models/scripts/eval_end2end_phone_net.py $feats/$subdir/feats-norm.JOB.scp \
...
echo "DONE END-TO-END PHONE NET EVAL JOB"
