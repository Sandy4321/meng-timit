...
#SBATCH -J eval_enhancement_md

echo "STARTING ENHANCEMENT MULTIDECODER EVAL JOB"
...
eval_log=$LOG_DIR/eval_enhancement_md.log
if [ -f $eval_log ]; then
    # Move old log
    mv $eval_log $LOG_DIR/eval_enhancement_md-$(date +"%F_%T%z").log
fi

# Decode and evaluate the enhancement multidecoder
for source_class in clean dirty; do
    ...
    for subdir in dev test; do
        ...
        dir=$LOG_DIR/enhancement_md/$subdir/$source_class
        mkdir -p $dir
        ...
        if [ ! -f $dir/lat.1.gz ]; then
            ...
            $UTILS/run.pl JOB=1:$num_jobs $dir/decode-JOB.log \
                python3 models/scripts/eval_enhancement_md.py $feats/$subdir/feats-norm.JOB.scp \
...
echo "DONE ENHANCEMENT MULTIDECODER EVAL JOB"
