...
#SBATCH -J eval_enhancement_net

echo "STARTING ENHANCEMENT NET EVAL JOB"
...
eval_log=$LOG_DIR/eval_enhancement_net.log
if [ -f $eval_log ]; then
    # Move old log
    mv $eval_log $LOG_DIR/eval_enhancement_net-$(date +"%F_%T%z").log
fi

# Decode and evaluate the enhancement network
for source_class in clean dirty; do
    ...
    for subdir in dev test; do
        ...
        dir=$LOG_DIR/enhancement_net/$subdir/$source_class
        mkdir -p $dir
        ...
        if [ ! -f $dir/lat.1.gz ]; then
            ...
            $UTILS/run.pl JOB=1:$num_jobs $dir/decode-JOB.log \
                python3 models/scripts/eval_enhancement_net.py $feats/$subdir/feats-norm.JOB.scp \
...
echo "DONE ENHANCEMENT NET EVAL JOB"
