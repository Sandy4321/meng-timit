...
#SBATCH -J augment_vanilla_phone

echo "STARTING VANILLA MULTITASK MULTIDECODER DATA AUGMENTATION JOB"

. ./path.sh
. ./cnn/base_config.sh
. ./cnn/phone_config.sh

# recipe_name=single_rir_vanilla_phone_3_16
if [ $# -eq 1 ]; then
    recipe_name=$1
else
    echo "Need to provide recipe name"
    exit 1
fi
...
augment_log=$LOG_DIR/augment_vanilla_phone.log
if [ -f $augment_log ]; then
    # Move old log
    mv $augment_log $LOG_DIR/augment_vanilla_phone-$(date +"%F_%T%z").log
fi

augment_dir=$AUGMENTED_DATA_DIR/vanilla_phone
mkdir -p $augment_dir/train
mkdir -p $augment_dir/dev
mkdir -p $augment_dir/test

python3 cnn/scripts/augment_vanilla_phone.py &> $augment_log
...
echo "DONE VANILLA MULTITASK MULTIDECODER DATA AUGMENTATION JOB"
