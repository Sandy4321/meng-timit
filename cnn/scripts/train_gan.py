...
# Set up real/fake discriminators for each decoder class
gan_optimizers = dict()
for decoder_class in decoder_classes:
...
def train(epoch, train_adversary=True, adversary_k=1):
    model.train()

    # Set up some bookkeeping on loss values
    decoder_class_losses = LossDict(decoder_classes, gan=True)
    batches_processed = 0
    ...
    for batch_idx, (all_feats, all_targets) in enumerate(train_loader):
        ...
        if train_adversary and (batches_processed % adversary_k == 0):
            ...
            for i in range(len(decoder_classes)):
                ...
                feats = feat_dict[decoder_class] 
                guesses = model.forward_gan(feats, decoder_class)
                ...
                other_feats = feat_dict[other_decoder_class] 
                transformed_feats = model.forward_decoder(other_feats, decoder_class)
                guesses = model.forward_gan(transformed_feats, decoder_class)
            ...
            for i in range(len(decoder_classes)):
                ...
                real_loss = discriminative_loss(guesses, labels_v)
                real_loss.backward()
                decoder_class_losses.add(decoder_class, {"real_gan_loss": real_loss.data[0]})
                ...
                fake_loss = discriminative_loss(guesses, labels_v)
                fake_loss.backward()
                decoder_class_losses.add(decoder_class, {"fake_gan_loss": fake_loss.data[0]})

            # Update gradients now that we've seen both real and fake data for both classes
            generator_optimizer.step()
...
print("Starting pre-training (epoch 0)!", flush=True)
pretrain_loss_dict = train(0, train_adversary=False)
print("\nPRETRAIN (EPOCH 0)", flush=True)
...
for epoch in range(1, epochs + 1):
    ...
    if is_best:
        ...
        state_obj = {
            ...
            "gan_optimizers": {dc: gan_optimizers[dc].state_dict() for dc in decoder_classes},
        }
