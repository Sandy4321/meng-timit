...
# all of "feats.scp", "wav.scp" [if present], "phones.scp" [if present], segments [if present]
...
for x in utt2spk spk2utt feats.scp text segments wav.scp cmvn.scp vad.scp \
    reco2file_and_channel spk2gender utt2lang utt2uniq utt2dur utt2num_frames phones.scp; do
  if [ -f $data/$x ]; then
...
function filter_utts {
  ...
  for x in feats.scp text segments utt2lang $maybe_wav phones.scp; do
    if [ -f $data/$x ]; then
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur utt2num_frames $maybe_wav $utt_extra_files phones.scp; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
