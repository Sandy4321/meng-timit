...
train_log=$LOG_DIR/train_vanilla_e2e.log
if [ -f $train_log ]; then
    # Move old log
    mv $train_log $LOG_DIR/train_vanilla_e2e-$(date +"%F_%T%z").log
fi

# Train model itself
if [ $stage -lt 1 ]; then
    echo "Training model..."
    python3 cnn/scripts/train_vanilla_e2e.py &> $train_log
fi
    ...
    for source_class in "${DECODER_CLASSES[@]}"; do
        ...
        for subdir in dev test; do
            ...
            if [ ! -f $dir/lat.1.gz ]; then
                ...
                python3 cnn/scripts/eval_vanilla_e2e.py $subdir $source_class \
                    | $UTILS/batch2ark.py \
